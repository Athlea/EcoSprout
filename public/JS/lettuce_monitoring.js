...
async function logWatering(pump, duration, mode, customTime = null, customDate = null) {
  const historyRef = ref(database, `wateringHistory/lettuce`)
  const countRef = ref(database, `wateringHistory/lettuce/count`)
  ...
  try {
    ...
    const entryRef = ref(database, `wateringHistory/lettuce/entry${count}`)
    ...
    console.log(`Watering logged for Lettuce: ${duration} min (${mode}) as entry${count}`)
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  autoWaterToggle.addEventListener("change", () => {
    ...
    if (isAutoWaterEnabled) {
      onValue(autoWaterRef.duration, (snapshot) => {
        if (snapshot.exists()) {
          logWatering("pump3", snapshot.val(), "Auto")
        }
      })
    }
  ...
  waterNowButton.addEventListener("click", async () => {
    ...
    try {
      ...
      await logWatering("pump3", duration, "Manual", formattedTime, formattedDate)
    } catch (error) {
